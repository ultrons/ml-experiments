...
class DataLoaderLite:
    def __init__(self, B, T, process_rank=0, num_processes=1):
        ...
        # At the init load tokens from the disk and store them in memory        
    ...
    def next_batch(self):
        B, T = self.B, self.T
        rank, num_processes = self.process_rank, self.num_processes
        
        buf = self.tokens[self.current_position:self.current_position + B * T + 1]
        self.current_position += num_processes * B * T
        x = buf[:-1].view(B,T)
        y = buf[1:].view(B,T)
